...
class AVOutputTV : public AVOutputBase {
    ...
    private:
		 ...
		 */
		int updateAVoutputTVParam(const std::string& action, const std::string& tr181ParamName, const capDetails_t& info, tvPQParameterIndex_t pqParamIndex, int level );
		int updateAVoutputTVParamImplementation( const std::string& action, const std::string& tr181ParamName, tvPQParameterIndex_t pqParamIndex, int level, const valueVectors_t& values, const std::vector<paramIndex_t>& skipTuples = {} );
		...
		tvError_t setDefaultAutoBacklightMode(std::string pqmode="none",std::string format="none",std::string source="none");
		template <typename T>
