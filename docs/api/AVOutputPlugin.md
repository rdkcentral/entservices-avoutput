<!-- Maintained in entservices-AVoutput with repository-specific deltas and compliance notes. -->
<a name="AVOutput_Plugin"></a>
# AVOutput Plugin

**Version: [1.1.0](../../CHANGELOG.md)**

An org.rdk.AVOutput plugin for Thunder framework.

### Table of Contents

- [Repository Scope and Compliance](#Repository_Scope_and_Compliance)
...
<a name="Repository_Scope_and_Compliance"></a>
# Repository Scope and Compliance

Scope:

- This document is the repository-owned API reference for org.rdk.AVOutput.
- Method exposure is derived from JSON-RPC registration in `plugin/AVOutputTV.cpp`.
- Notification exposure is derived from `sendNotify(...)` usage in `plugin/AVOutputTV.cpp` and `plugin/AVOutputTVHelper.cpp`.

Open source compliance notes:

- License files: `LICENSE`, `COPYING`
- Third-party/legal notices: `NOTICE`
- Contribution process: `CONTRIBUTING.md`
- API maintenance guide: `docs/api/AVOutputDocMaintenance.md`
- API changes should update this document in the same change set that modifies method registration or emitted notifications.
...
The plugin is designed to be loaded and executed within the Thunder framework. For more information about the framework refer to [[Thunder](userguide/aat.md#Thunder)].
...
The following methods are documented for the org.rdk.AVOutput plugin:
...
| [getBacklightCapsV2](#getBacklightCapsV2) | Gets the capability and context map for TV user backlight |
| [getBrightnessCapsV2](#getBrightnessCapsV2) | Gets the capability and context map for TV brightness |
| [getContrastCapsV2](#getContrastCapsV2) | Gets the capability and context map for TV contrast |
| [getSharpnessCapsV2](#getSharpnessCapsV2) | Gets the capability and context map for TV sharpness |
| [getSaturationCapsV2](#getSaturationCapsV2) | Gets the capability and context map for TV saturation |
| [getHueCapsV2](#getHueCapsV2) | Gets the capability and context map for TV hue |
| [getLowLatencyStateCapsV2](#getLowLatencyStateCapsV2) | Gets the capability and context map for low latency state |
| [getColorTemperatureCapsV2](#getColorTemperatureCapsV2) | Gets the color temperature options and context map |
| [getBacklightDimmingModeCapsV2](#getBacklightDimmingModeCapsV2) | Gets backlight dimming mode options and context map |
| [getZoomModeCapsV2](#getZoomModeCapsV2) | Gets zoom mode options and context map |
| [getPictureModeCapsV2](#getPictureModeCapsV2) | Gets picture mode options and context map |
| [getAutoBacklightModeCapsV2](#getAutoBacklightModeCapsV2) | Gets auto backlight mode options and context map |
| [getCMSCapsV2](#getCMSCapsV2) | Gets CMS capability ranges, supported colors/components and context map |
...
<a name="getBacklightCapsV2"></a>
## *getBacklightCapsV2*

Gets the backlight capability range and supported context map for picture mode, video format, and video source.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.rangeInfo | object | Numeric capability range |
| result.rangeInfo.from | number | Lower-inclusive value |
| result.rangeInfo.to | number | Upper-inclusive value |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
<a name="getBrightnessCapsV2"></a>
## *getBrightnessCapsV2*

Gets the brightness capability range and supported context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getBrightnessCapsV2"
}
```
...
<a name="getContrastCapsV2"></a>
## *getContrastCapsV2*

Gets the contrast capability range and supported context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getContrastCapsV2"
}
```
...
<a name="getSharpnessCapsV2"></a>
## *getSharpnessCapsV2*

Gets the sharpness capability range and supported context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getSharpnessCapsV2"
}
```
...
<a name="getSaturationCapsV2"></a>
## *getSaturationCapsV2*

Gets the saturation capability range and supported context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getSaturationCapsV2"
}
```
...
<a name="getHueCapsV2"></a>
## *getHueCapsV2*

Gets the hue capability range and supported context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getHueCapsV2"
}
```
...
<a name="getLowLatencyStateCapsV2"></a>
## *getLowLatencyStateCapsV2*

Gets low latency state capability range and supported context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getLowLatencyStateCapsV2"
}
```
...
<a name="getColorTemperatureCapsV2"></a>
## *getColorTemperatureCapsV2*

Gets supported color temperature options and context map.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.options | array | Supported color temperature options |
| result.options[#] | string | Option value (for example Standard, Warm, Cold, UserDefined) |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getColorTemperatureCapsV2"
}
```
...
<a name="getBacklightDimmingModeCapsV2"></a>
## *getBacklightDimmingModeCapsV2*

Gets supported backlight dimming mode options and context map.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.options | array | Supported dimming mode options |
| result.options[#] | string | Option value |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getBacklightDimmingModeCapsV2"
}
```
...
<a name="getZoomModeCapsV2"></a>
## *getZoomModeCapsV2*

Gets supported zoom mode options and context map.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.options | array | Supported zoom mode options |
| result.options[#] | string | Option value |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getZoomModeCapsV2"
}
```
...
<a name="getPictureModeCapsV2"></a>
## *getPictureModeCapsV2*

Gets supported picture mode options and context map.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.options | array | Supported picture mode options |
| result.options[#] | string | Option value |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getPictureModeCapsV2"
}
```
...
<a name="getAutoBacklightModeCapsV2"></a>
## *getAutoBacklightModeCapsV2*

Gets supported auto backlight mode options and context map.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.options | array | Supported auto backlight mode options (for example Manual, Ambient, Eco) |
| result.options[#] | string | Option value |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getAutoBacklightModeCapsV2"
}
```
...
<a name="getCMSCapsV2"></a>
## *getCMSCapsV2*

Gets CMS capability ranges, supported colors/components, and context map.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.rangeHue | object | Hue range |
| result.rangeHue.from | number | Lower-inclusive hue value |
| result.rangeHue.to | number | Upper-inclusive hue value |
| result.rangeSaturation | object | Saturation range |
| result.rangeSaturation.from | number | Lower-inclusive saturation value |
| result.rangeSaturation.to | number | Upper-inclusive saturation value |
| result.rangeLuma | object | Luma range |
| result.rangeLuma.from | number | Lower-inclusive luma value |
| result.rangeLuma.to | number | Upper-inclusive luma value |
| result.color | array | Supported CMS colors |
| result.color[#] | string | Color option |
| result.component | array | Supported CMS components |
| result.component[#] | string | Component option |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getCMSCapsV2"
}
```
...
<a name="get2PointWBCapsV2"></a>
## *get2PointWBCapsV2*

Gets 2-point white-balance capability ranges, controls, colors, and context map.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.rangeGain | object | Gain range |
| result.rangeGain.from | number | Lower-inclusive gain value |
| result.rangeGain.to | number | Upper-inclusive gain value |
| result.rangeOffset | object | Offset range |
| result.rangeOffset.from | number | Lower-inclusive offset value |
| result.rangeOffset.to | number | Upper-inclusive offset value |
| result.control | array | Supported controls |
| result.control[#] | string | Control value |
| result.color | array | Supported colors |
| result.color[#] | string | Color value |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.get2PointWBCapsV2"
}
```
...
<a name="getDolbyVisionCalibrationCaps"></a>
## *getDolbyVisionCalibrationCaps*

Gets Dolby Vision calibration capability details. Returned values are platform and profile dependent.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.rangeTmax | object | Valid value range for Tmax |
| result.rangeTmax.from | number | Lower-inclusive Tmax value |
| result.rangeTmax.to | number | Upper-inclusive Tmax value |
| result.rangeTmin | object | Valid value range for Tmin |
| result.rangeTmin.from | number | Lower-inclusive Tmin value |
| result.rangeTmin.to | number | Upper-inclusive Tmin value |
| result.rangeTgamma | object | Valid value range for Tgamma |
| result.rangeTgamma.from | number | Lower-inclusive Tgamma value |
| result.rangeTgamma.to | number | Upper-inclusive Tgamma value |
| result.rangeRx | object | Valid value range for Rx |
| result.rangeRx.from | number | Lower-inclusive Rx value |
| result.rangeRx.to | number | Upper-inclusive Rx value |
| result.rangeRy | object | Valid value range for Ry |
| result.rangeRy.from | number | Lower-inclusive Ry value |
| result.rangeRy.to | number | Upper-inclusive Ry value |
| result.rangeGx | object | Valid value range for Gx |
| result.rangeGx.from | number | Lower-inclusive Gx value |
| result.rangeGx.to | number | Upper-inclusive Gx value |
| result.rangeGy | object | Valid value range for Gy |
| result.rangeGy.from | number | Lower-inclusive Gy value |
| result.rangeGy.to | number | Upper-inclusive Gy value |
| result.rangeBx | object | Valid value range for Bx |
| result.rangeBx.from | number | Lower-inclusive Bx value |
| result.rangeBx.to | number | Upper-inclusive Bx value |
| result.rangeBy | object | Valid value range for By |
| result.rangeBy.from | number | Lower-inclusive By value |
| result.rangeBy.to | number | Upper-inclusive By value |
| result.rangeWx | object | Valid value range for Wx |
| result.rangeWx.from | number | Lower-inclusive Wx value |
| result.rangeWx.to | number | Upper-inclusive Wx value |
| result.rangeWy | object | Valid value range for Wy |
| result.rangeWy.from | number | Lower-inclusive Wy value |
| result.rangeWy.to | number | Upper-inclusive Wy value |
| result.context | array | Dolby Vision picture modes supported for calibration |
| result.context[#] | string | Picture mode value |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getDolbyVisionCalibrationCaps"
}
```
...
<a name="getSDRGammaCaps"></a>
## *getSDRGammaCaps*

Gets supported SDR gamma options and context map. SDR gamma is only supported for the SDR video format; the context map will always contain only `"SDR"` video format entries.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.options | array | Supported SDR gamma options |
| result.options[#] | string | Option value (1.8, 1.9, 2.0, 2.1, 2.2, 2.3, 2.4, BT.1886) |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getSDRGammaCaps",
    "params": {}
}
```
...
<a name="getSDRGamma"></a>
## *getSDRGamma*

Gets SDR gamma for the requested or current context.
...
| Name | Type | Description |
| :-------- | :-------- | :-------- |
| params | object | Optional context parameter object |
| params?.pictureMode | string | Optional picture mode. If omitted, current is used |
| params?.videoSource | string | Optional video source. If omitted, current is used |
| params?.videoFormat | string | Optional video format. If omitted, current is used |
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.sdrGamma | string | SDR gamma value |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getSDRGamma",
    "params": {
        "pictureMode": "Movie",
        "videoSource": "HDMI1",
        "videoFormat": "SDR"
...
<a name="setSDRGamma"></a>
## *setSDRGamma*

Sets SDR gamma for the requested or current context.
...
| params.sdrGamma | string | SDR gamma value. Valid values are from [getSDRGammaCaps](#getSDRGammaCaps) |
| params?.pictureMode | string | Optional picture mode context |
| params?.videoSource | string | Optional video source context |
| params?.videoFormat | string | Optional video format context |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.setSDRGamma",
    "params": {
        "pictureMode": [
            "Current"
        ],
...
<a name="resetSDRGamma"></a>
## *resetSDRGamma*

Resets SDR gamma to default for the requested or current context.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.resetSDRGamma",
    "params": {
        "pictureMode": [
            "Current"
        ],
...
<a name="getPrecisionDetailCaps"></a>
## *getPrecisionDetailCaps*

Gets precision detail capability range and context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getPrecisionDetailCaps",
    "params": {}
}
```
...
<a name="getPrecisionDetail"></a>
## *getPrecisionDetail*

Gets precision detail value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.precisionDetail | number | Precision detail value |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getPrecisionDetail",
    "params": {
        "pictureMode": "IQ",
        "videoFormat": "DV"
    }
...
<a name="setPrecisionDetail"></a>
## *setPrecisionDetail*

Sets precision detail value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| params | object | Parameter object |
| params.precisionDetail | number | Value to set. Valid range is from [getPrecisionDetailCaps](#getPrecisionDetailCaps) |
| params?.pictureMode | string | Optional picture mode context |
| params?.videoSource | string | Optional video source context |
| params?.videoFormat | string | Optional video format context |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.setPrecisionDetail",
    "params": {
        "pictureMode": [
            "IQ",
            "Bright"
...
<a name="resetPrecisionDetail"></a>
## *resetPrecisionDetail*

Resets precision detail to default for the requested or current context.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.resetPrecisionDetail",
    "params": {
        "pictureMode": [
            "Global"
        ],
...
<a name="getLocalContrastEnhancementCaps"></a>
## *getLocalContrastEnhancementCaps*

Gets local contrast enhancement capability range and context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getLocalContrastEnhancementCaps",
    "params": {}
}
```
...
<a name="getLocalContrastEnhancement"></a>
## *getLocalContrastEnhancement*

Gets local contrast enhancement value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.localContrastEnhancement | number | Local contrast enhancement value |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getLocalContrastEnhancement",
    "params": {
...
```json
{
    ...
    "id": 42,
    "result": {
        "localContrastEnhancement": 2,
        "success": true
    }
}
```

<a name="setLocalContrastEnhancement"></a>
## *setLocalContrastEnhancement*

Sets local contrast enhancement value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| params | object | Parameter object |
| params.localContrastEnhancement | number | Value to set. Valid range is from [getLocalContrastEnhancementCaps](#getLocalContrastEnhancementCaps) |
| params?.pictureMode | string | Optional picture mode context |
| params?.videoSource | string | Optional video source context |
| params?.videoFormat | string | Optional video format context |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.setLocalContrastEnhancement",
    "params": {
        "pictureMode": [
            "Sports",
            "Theater"
...
<a name="resetLocalContrastEnhancement"></a>
## *resetLocalContrastEnhancement*

Resets local contrast enhancement to default for the requested or current context.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.resetLocalContrastEnhancement",
    "params": {
        "pictureMode": [
            "Current"
        ],
...
<a name="getMPEGNoiseReductionCaps"></a>
## *getMPEGNoiseReductionCaps*

Gets MPEG noise reduction capability range and context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getMPEGNoiseReductionCaps",
    "params": {}
}
```
...
<a name="getMPEGNoiseReduction"></a>
## *getMPEGNoiseReduction*

Gets MPEG noise reduction value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.mpegNoiseReduction | number | MPEG noise reduction value |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getMPEGNoiseReduction",
    "params": {
        "pictureMode": "Standard",
        "videoFormat": "HDR10"
    }
...
```json
{
    ...
    "id": 42,
    "result": {
        "mpegNoiseReduction": 2,
        "success": true
    }
}
```

<a name="setMPEGNoiseReduction"></a>
## *setMPEGNoiseReduction*

Sets MPEG noise reduction value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| params | object | Parameter object |
| params.mpegNoiseReduction | number | Value to set. Valid range is from [getMPEGNoiseReductionCaps](#getMPEGNoiseReductionCaps) |
| params?.pictureMode | string | Optional picture mode context |
| params?.videoSource | string | Optional video source context |
| params?.videoFormat | string | Optional video format context |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.setMPEGNoiseReduction",
    "params": {
        "pictureMode": [
            "Sports",
            "Theater"
...
<a name="resetMPEGNoiseReduction"></a>
## *resetMPEGNoiseReduction*

Resets MPEG noise reduction to default for the requested or current context.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.resetMPEGNoiseReduction",
    "params": {
        "pictureMode": [
            "Current"
        ],
...
<a name="getDigitalNoiseReductionCaps"></a>
## *getDigitalNoiseReductionCaps*

Gets digital noise reduction capability range and context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getDigitalNoiseReductionCaps",
    "params": {}
}
```
...
<a name="getDigitalNoiseReduction"></a>
## *getDigitalNoiseReduction*

Gets digital noise reduction value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.digitalNoiseReduction | number | Digital noise reduction value |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getDigitalNoiseReduction",
    "params": {
        "pictureMode": "Standard",
        "videoFormat": "HDR10"
    }
...
```json
{
    ...
    "id": 42,
    "result": {
        "digitalNoiseReduction": 2,
        "success": true
    }
}
```

<a name="setDigitalNoiseReduction"></a>
## *setDigitalNoiseReduction*

Sets digital noise reduction value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| params | object | Parameter object |
| params.digitalNoiseReduction | number | Value to set. Valid range is from [getDigitalNoiseReductionCaps](#getDigitalNoiseReductionCaps) |
| params?.pictureMode | string | Optional picture mode context |
| params?.videoSource | string | Optional video source context |
| params?.videoFormat | string | Optional video format context |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.setDigitalNoiseReduction",
    "params": {
        "pictureMode": [
            "Sports",
            "Theater"
...
<a name="resetDigitalNoiseReduction"></a>
## *resetDigitalNoiseReduction*

Resets digital noise reduction to default for the requested or current context.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.resetDigitalNoiseReduction",
    "params": {
        "pictureMode": [
            "Current"
        ],
...
<a name="getMEMCCaps"></a>
## *getMEMCCaps*

Gets MEMC capability range and context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getMEMCCaps",
    "params": {}
}
```
...
Gets MEMC value for the requested or current context.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getMEMC",
    "params": {
        "pictureMode": "Standard",
        "videoSource": "Current",
        "videoFormat": "HDR10"
...
Sets MEMC value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| params | object | Parameter object |
| params.memc | number | Value to set. Valid range is from [getMEMCCaps](#getMEMCCaps) |
| params?.pictureMode | string | Optional picture mode context |
| params?.videoSource | string | Optional video source context |
| params?.videoFormat | string | Optional video format context |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.setMEMC",
    "params": {
        "pictureMode": [
            "Sports",
            "Theater"
...
Resets MEMC to default for the requested or current context.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.resetMEMC",
    "params": {
        "pictureMode": [
            "Current"
        ],
...
<a name="getAISuperResolutionCaps"></a>
## *getAISuperResolutionCaps*

Gets AI super resolution capability range and context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getAISuperResolutionCaps",
    "params": {}
}
```
...
<a name="getAISuperResolution"></a>
## *getAISuperResolution*

Gets AI super resolution value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.aiSuperResolution | number | AI super resolution value |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getAISuperResolution",
    "params": {
        "pictureMode": "Standard",
        "videoSource": "Current",
        "videoFormat": "HDR10"
...
<a name="setAISuperResolution"></a>
## *setAISuperResolution*

Sets AI super resolution value for the requested or current context.
...
| :-------- | :-------- | :-------- |
| params | object | Parameter object |
| params.aiSuperResolution | number | Value to set. Valid range is from [getAISuperResolutionCaps](#getAISuperResolutionCaps) |
| params?.pictureMode | string | Optional picture mode context |
| params?.videoSource | string | Optional video source context |
| params?.videoFormat | string | Optional video format context |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.setAISuperResolution",
    "params": {
        "pictureMode": [
            "Sports",
            "Theater"
...
<a name="resetAISuperResolution"></a>
## *resetAISuperResolution*

Resets AI super resolution to default for the requested or current context.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.resetAISuperResolution",
    "params": {
        "pictureMode": [
            "Current"
        ],
...
<a name="getBacklightDimmingLevelCaps"></a>
## *getBacklightDimmingLevelCaps*

Gets backlight dimming level capability range and context map.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getBacklightDimmingLevelCaps"
}
```
...
<a name="getBacklightDimmingLevel"></a>
## *getBacklightDimmingLevel*

Gets backlight dimming level for the requested or current context.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.dimmingLevel | number | Backlight dimming level |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getBacklightDimmingLevel",
    "params": {
        "pictureMode": "Sports",
        "videoSource": "HDMI1",
        "videoFormat": "SDR"
...
<a name="setBacklightDimmingLevel"></a>
## *setBacklightDimmingLevel*

Sets backlight dimming level for the requested or current context.
...
| :-------- | :-------- | :-------- |
| params | object | Parameter object |
| params.dimmingLevel | number | Value to set. Valid range is from [getBacklightDimmingLevelCaps](#getBacklightDimmingLevelCaps) |
| params?.pictureMode | string | Optional picture mode context |
| params?.videoSource | string | Optional video source context |
| params?.videoFormat | string | Optional video format context |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.setBacklightDimmingLevel",
    "params": {
        "dimmingLevel": 50
    }
}
...
<a name="resetBacklightDimmingLevel"></a>
## *resetBacklightDimmingLevel*

Resets backlight dimming level to default for the requested or current context.
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.resetBacklightDimmingLevel",
    "params": {
        "pictureMode": "Sports",
        "videoSource": "HDMI1",
        "videoFormat": "SDR"
...
<a name="getSupportedDolbyVisionModes"></a>
## *getSupportedDolbyVisionModes*

Gets the list of Dolby Vision modes supported by the platform.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.supportedDVModes | array | Supported Dolby Vision modes |
| result.supportedDVModes[#] | string | Dolby Vision mode name |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getSupportedDolbyVisionModes"
}
```
...
<a name="getSupportedPictureModes"></a>
## *getSupportedPictureModes*

Gets the list of picture modes supported by the platform.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.supportedPictureModes | array | Supported picture modes |
| result.supportedPictureModes[#] | string | Picture mode name |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getSupportedPictureModes"
}
```
...
<a name="getMultiPointWBCaps"></a>
## *getMultiPointWBCaps*

Gets multipoint white-balance capability details and context map.
...
| result | object |  |
| result.platformSupport | boolean | true if supported by platform |
| result.numHalMatrixPoints | number | Number of HAL matrix points |
| result.rgbMin | number | Minimum RGB value |
| result.rgbMax | number | Maximum RGB value |
| result.numUiMatrixPoints | number | Number of UI matrix points |
| result.uiMatrixPositions | array | UI matrix positions |
| result.uiMatrixPositions[#] | number | Position value |
| result.context | object | Context map keyed by picture mode -> video format -> supported video sources |
| result.success | boolean | Whether the request succeeded |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.getMultiPointWBCaps",
    "params": {}
}
```
...
<a name="signalFilmMakerMode"></a>
## *signalFilmMakerMode*

Triggers FilmMaker mode signaling and emits a corresponding content-change notification.

### Events

- [onVideoContentChanged](#onVideoContentChanged)
...
| :-------- | :-------- | :-------- |
| params | object | Parameter object |
| params.signalFilmMakerMode | string | FilmMaker mode signal value. "ON" enables FilmMaker mode signaling, any other value disables it |
...
```json
{
    "jsonrpc": "2.0",
    "id": 42,
    "method": "org.rdk.AVOutput.signalFilmMakerMode",
    "params": {
        "signalFilmMakerMode": "ON"
    }
}
```
...
```json
{
    ...
    "result": {
        "platformSupport": true,
        "componentSaturationRangeInfo": {
            "from": 0,
            "to": 100
        },
        "componentHueRangeInfo": {
            "from": 0,
            "to": 100
        },
        "componentLumaRangeInfo": {
            "from": 0,
            "to": 30
        },
        "pictureModeInfo": [
...
| result.rangeInfo | object | Range of values for the low latency state |
| result.rangeInfo.from | number | The lower-inclusive range value for result.lowLatencyState |
| result.rangeInfo.to | number | The higher-inclusive range value for result.lowLatencyState |
...
| params?.videoFormat | string | <sup>*(optional)*</sup> The picture format that the HDR mode should be fetched for. e.g. "Current", "HDR10", "HLG". "Current" - the current format of the current primary active source. When there is no active video playback or input port selected, "Current" refers to "SDR" video format as default. Other valid values for a platform can be obtained from options field of [getVideoFormatCaps](#getVideoFormatCaps).<br><br>The caller can pass a single valid value for this parameter. If omitted the default value assumed is "Current" |
...
| result.success | boolean | Whether the request succeeded. If `platformSupport` field of [getHDRModeCaps](#getHDRModeCaps) returns false, then this API will always return false |
...
| result?.pictureModeInfo[#] | string | <sup>*(optional)*</sup>  |
| result?.videoSourceInfo | array | <sup>*(optional)*</sup> Array of video sources that the HDR mode could be customised for. These are few examples of video sources available in this array: ["HDMI1", "HDMI2", "HDMI3", "IP"]. This API will return the entire list of video sources returned by [getVideoSourceCaps](#getVideoSourceCaps), or a subset of it. If HDR mode cannot be customised for video sources, then this property is not returned |
| result?.videoSourceInfo[#] | string | <sup>*(optional)*</sup>  |
...
Notifications are autonomous events, triggered by the internals of the implementation, and broadcasted via JSON-RPC to all registered observers. Refer to [[Thunder](userguide/aat.md#Thunder)] for information on how to register for a notification.

The following events are documented for the org.rdk.AVOutput plugin:
...
| [gameModeEvent](#gameModeEvent) | Event notification for low-latency mode transitions (game mode on/off) |
...
<a name="gameModeEvent"></a>
## *gameModeEvent*

Event notification for low-latency mode changes.
...
| :-------- | :-------- | :-------- |
| params | object |  |
| params.lowLatencyMode | boolean | true when low-latency/game mode is enabled, false when disabled |
...
{
    "jsonrpc": "2.0",
    "method": "client.events.gameModeEvent",
    "params": {
        "lowLatencyMode": true
    }
}
