As part of rdkservices open source activity and logical grouping of services into various entservices-* repos, the below listed changes to L1 and L2 tests are effective henceforth.
...
Hence, any modifications/additions related to mocks should be committed to entservices-testframework repo @ rdkcentral and any modifications/additions related to test case should be committed to Test directory of corresponding entservices repo.
...
```
This ensures everything remains intact at repo level across multiple related plugins when there is a new change.
...
NOTE: By default test-trigger.yml will trigger all tests (L1, L2 and etc.) in parallel, if you want any one test alone to be triggered/verified then remove the other trigger rules from the tests-trigger.yml
```
